...
const CATEGORY_COLORS = {
  urgent:   { bg: 'bg-soft-stone', text: 'text-ink', border: 'border-hairline', dot: 'bg-ink' },
  exam:     { bg: 'bg-soft-stone', text: 'text-ink', border: 'border-hairline', dot: 'bg-ink' },
  class:    { bg: 'bg-soft-stone', text: 'text-ink', border: 'border-hairline', dot: 'bg-ink' },
  academic: { bg: 'bg-soft-stone', text: 'text-ink', border: 'border-hairline', dot: 'bg-ink' },
  general:  { bg: 'bg-soft-stone', text: 'text-ink', border: 'border-hairline', dot: 'bg-ink' },
}
...
function Spinner() {
  return (
    <div className="flex items-center justify-center py-24">
      <div className="w-7 h-7 border-[2px] border-ink border-t-transparent rounded-full animate-spin" />
    </div>
  )
}
...
function EmptyState({ message }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center border border-hairline border-dashed rounded-lg bg-soft-stone/20">
      ...
      <p className="font-sans text-[15px] text-muted">{message}</p>
    </div>
  )
}
...
function ErrorState() {
  return (
    <div className="text-center py-16 border border-hairline border-dashed rounded-lg bg-soft-stone/20">
      <Bell size={24} className="mx-auto text-signature-coral mb-3" />
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-center gap-2 flex-wrap">
          {isPinned && <Pin size={14} className="text-signature-coral" />}
          <span
            className={`inline-flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wider px-2.5 py-1 rounded-md border ${color.bg} ${color.text} ${color.border}`}
          >
            <span className={`w-1.5 h-1.5 rounded-full ${color.dot}`} />
            {categoryLabel(ann.category)}
          </span>
          {isPinned && (
            <span className="font-mono text-[10px] font-medium uppercase tracking-wider text-signature-coral">
              Pinned
            </span>
          )}
        </div>
      ...
      <h3 className="font-display font-medium text-[17px] text-ink leading-snug mb-1 line-clamp-2">
        {ann.title || 'Untitled Announcement'}
      </h3>

      {preview && (
        <p className="font-sans text-[14px] text-body leading-relaxed mb-3 line-clamp-3 break-words">
          {preview}
        </p>
      )}
...
function AnnouncementDetail({ detail, loading, error, user, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl max-h-[85vh] bg-white border border-hairline rounded-xl shadow-lg flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {loading ? (
          ...
          <div className="p-6">
            <p className="font-sans text-[14px] text-signature-coral">
          ...
          <>
            <div className="px-6 pt-5 pb-4 border-b border-hairline">
              <div className="flex items-start justify-between gap-3">
                ...
                {user && (
                  <span
                    className={`font-mono text-[11px] font-medium uppercase tracking-wider px-2.5 py-1 rounded-full ${
                      isRead
                        ? 'text-signature-forest bg-signature-mint'
                        : 'text-signature-coral bg-signature-cream'
                    }`}
                  >
                    {isRead ? 'Read' : 'New'}
                  </span>
              ...
              <h2 className="font-display text-[22px] font-normal text-ink mt-3 leading-snug break-words">
          ...
          {CATEGORIES.map((c) => {
            ...
            return (
              <button
                ...
                className={`font-mono text-[12px] font-medium uppercase tracking-wider px-3.5 py-1.5 rounded-full transition-all whitespace-nowrap ${
                  active
                    ? 'bg-primary text-white'
                    : 'text-muted hover:text-ink bg-soft-stone hover:bg-soft-stone/80'
                }`}
              >
                {c.label}
              </button>
