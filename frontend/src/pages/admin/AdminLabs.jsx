  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <h1 className="font-[Inter,system-ui,sans-serif] font-semibold text-[28px] tracking-tight text-ink">Department Laboratories</h1>
      ...
      {showForm && (
        <div className="border border-divider-soft bg-white p-6 mb-8 rounded-xl shadow-sm">
          <h2 className="font-[Inter,system-ui,sans-serif] font-semibold text-[18px] text-ink mb-6">{editing ? 'Edit Laboratory' : 'New Laboratory'}</h2>
          <div className="grid sm:grid-cols-2 gap-5">
            ...
            <div className="sm:col-span-2">
              <label className="block font-[Inter,system-ui,sans-serif] text-[14px] font-medium text-ink-muted-80 mb-1">Equipment (comma separated)</label>
            ...
            <div className="sm:col-span-2">
              <label className="block font-[Inter,system-ui,sans-serif] text-[14px] font-medium text-ink-muted-80 mb-1">Lab Image</label>
      ...
      {isLoading ? <p className="font-[Inter,system-ui,sans-serif] text-ink-muted-80 text-[15px]">Loading laboratories�</p>
        ...
        : (
          <div className="border border-divider-soft bg-white rounded-xl overflow-hidden shadow-sm divide-y divide-hairline">
            {labs.map(l => (
              <div key={l._id} className="flex items-center gap-4 px-4 sm:px-6 py-3 sm:py-4 hover:bg-[#fff]-parchment transition-colors">
                {l.image ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-[Inter,system-ui,sans-serif] text-[13px] text-ink-muted-80 mt-1 line-clamp-2">{l.desc}</p>
                  {l.equip && l.equip.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {l.equip.map(eq => (
                        <span key={eq} className="font-mono text-[10px] font-medium uppercase px-2 py-0.5 rounded-full bg-soft-stone text-ink border border-hairline">{eq}</span>
                      ))}
                    </div>
                  )}
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => openEdit(l)} className="font-[Inter,system-ui,sans-serif] text-[13px] font-medium text-primary bg-primary/10 hover:bg-primary/20 transition-colors px-3 py-1.5 rounded-md">Edit</button>
