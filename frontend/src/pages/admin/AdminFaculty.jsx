  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <h1 className="font-[Inter,system-ui,sans-serif] font-semibold text-[28px] tracking-tight text-ink">Faculty Directory</h1>
      ...
      {showForm && (
        <div className="border border-divider-soft bg-white p-6 mb-8 rounded-xl shadow-sm">
          <h2 className="font-[Inter,system-ui,sans-serif] font-semibold text-[18px] text-ink mb-6">{editing ? 'Edit Faculty' : 'New Faculty Member'}</h2>
          <div className="grid sm:grid-cols-2 gap-5">
            ...
            <div>
              <label className="block font-[Inter,system-ui,sans-serif] text-[14px] font-medium text-ink-muted-80 mb-1">Designation *</label>
              <input value={form.designation} onChange={set('designation')} className="input w-full" placeholder="e.g. Associate Professor" />
            </div>
            <div>
              <label className="block font-[Inter,system-ui,sans-serif] text-[14px] font-medium text-ink-muted-80 mb-1">Qualification</label>
              <input value={form.qualification} onChange={set('qualification')} className="input w-full" placeholder="e.g. Ph.D. (EE)" />
            </div>
            <div className="sm:col-span-2">
              <label className="block font-[Inter,system-ui,sans-serif] text-[14px] font-medium text-ink-muted-80 mb-1">Specialization</label>
            ...
            <div>
              <label className="block font-[Inter,system-ui,sans-serif] text-[14px] font-medium text-ink-muted-80 mb-1">Photo URL (optional)</label>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <label htmlFor="isHOD" className="font-[Inter,system-ui,sans-serif] text-[14px] text-ink">Mark as HOD</label>
            </div>
          </div>
      ...
      {isLoading ? <p className="font-[Inter,system-ui,sans-serif] text-ink-muted-80 text-[15px]">Loading faculty�</p>
        ...
        : (
          <div className="border border-divider-soft bg-white rounded-xl overflow-hidden shadow-sm">
            {faculty.map(f => (
              <div key={f._id} className="flex items-center gap-4 px-4 sm:px-6 py-3 sm:py-4 border-b border-divider-soft last:border-b-0 hover:bg-[#fff]-parchment transition-colors">
                <div className="w-12 h-12 rounded-22px bg-soft-stone flex items-center justify-center flex-shrink-0 overflow-hidden border border-hairline shadow-sm">
                  {f.photo ? <img src={f.photo} alt={f.name} className="w-full h-full object-cover" />
                    : <span className="font-[Inter,system-ui,sans-serif] font-medium text-[18px] text-ink">{initials(f.name)}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[15px] font-medium text-ink flex items-center gap-2">
                    {f.name}
                    {f.isHOD && <span className="font-mono text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded-full bg-deep-green text-white">HOD</span>}
                  </p>
                  <p className="font-mono text-[12px] text-slate uppercase tracking-wider">{f.designation}</p>
                  <p className="font-[Inter,system-ui,sans-serif] text-[13px] text-ink-muted-80 truncate">{f.specialization || f.qualification}</p>
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => openEdit(f)} className="font-[Inter,system-ui,sans-serif] text-[13px] font-medium text-primary bg-primary/10 hover:bg-primary/20 transition-colors px-3 py-1.5 rounded-md">Edit</button>
