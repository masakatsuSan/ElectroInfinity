...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: 'Notices',    count: nData?.total ?? '—',              to: '/admin/notices',    badge: 'Announcements' },
  ...
  return (
    <div className="space-y-10 max-w-6xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        {stats.map(s => (
          <Link
            key={s.label}
            to={s.to}
            className="border border-hairline bg-canvas hover:bg-soft-stone/50 rounded-2xl p-5 transition-all shadow-card flex flex-col justify-between group"
          >
            <div>
              <span className="font-mono text-[11px] uppercase tracking-wider text-slate font-medium block mb-2">
                {s.badge}
              </span>
              <div className="font-display font-bold text-[36px] leading-none text-ink group-hover:text-action-blue transition-colors">
                {s.count}
              </div>
            </div>
            <div className="font-sans text-[13px] font-semibold text-body-muted mt-4 pt-3 border-t border-hairline">
              {s.label} →
            </div>
          </Link>
        ))}
      ...
      {/* Recent notices (Research-table style) */}
      <div className="border border-hairline bg-canvas rounded-2xl p-6 shadow-card space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-hairline">
          <h3 className="font-display text-[20px] font-bold text-ink">Latest Department Notices</h3>
          <Link to="/admin/notices" className="button-pill-outline text-[13px]">
            View All Notices →
          </Link>
        </div>
        ...
        {recentNotices.length > 0 ? (
          <div className="divide-y divide-hairline">
            {recentNotices.map(n => (
              <div key={n._id} className="py-4 flex items-center justify-between gap-4 hover:bg-soft-stone/30 transition-colors px-2 rounded-lg">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <span className="font-sans text-[14px] font-medium text-ink truncate">{n.title}</span>
                </div>
          ...
          <p className="text-body-muted text-[14px] py-8 text-center">No notices published yet.</p>
        )}
      </div>
    </div>
  )
