  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <h1 className="font-[Inter,system-ui,sans-serif] font-semibold text-[28px] tracking-tight text-ink">Calendar</h1>
      ...
      {showForm && (
        <div className="border border-divider-soft bg-white p-6 mb-8 rounded-xl shadow-sm">
          <h2 className="font-[Inter,system-ui,sans-serif] font-semibold text-[18px] text-ink mb-6">{editing ? 'Edit Entry' : 'New Calendar Entry'}</h2>
      ...
      {isLoading ? <p className="font-[Inter,system-ui,sans-serif] text-ink-muted-80 text-[15px]">Loading calendar…</p>
        : entries.length === 0 ? <p className="font-[Inter,system-ui,sans-serif] text-ink-muted-80 text-[15px]">No calendar entries. Create one above.</p>
        : (
          <div className="border border-divider-soft bg-white rounded-xl overflow-hidden shadow-sm">
            {entries.map(e => (
              <div key={e._id} className="flex items-start gap-4 px-4 sm:px-6 py-3 sm:py-4 border-b border-divider-soft last:border-b-0 hover:bg-[#fff]-parchment transition-colors">
                <span className={`font-mono text-[11px] font-medium uppercase px-2.5 py-1 rounded-md border flex-shrink-0 ${TYPE_COLORS[e.type] || TYPE_COLORS.other}`}>
                  {e.type}
                </span>
                <div className="flex-1 min-w-0">
                  ...
                  {e.description && <p className="font-[Inter,system-ui,sans-serif] text-[13px] text-ink-muted-80 mt-1 line-clamp-2">{e.description}</p>}
                </div>
