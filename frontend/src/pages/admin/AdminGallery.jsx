...
const CATS = ['workshop', 'event', 'lab', 'campus', 'other']
...
export default function AdminGallery() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setForm(BLANK); setEditing(null); setShowForm(false); setFile(null)
      showToast(editing ? 'Gallery photo updated successfully!' : 'Gallery photo uploaded successfully!')
    },
  ...
  const openCreate = () => { setEditing(null); setForm(BLANK); setShowForm(true); setFile(null) }
  const openEdit = (p) => {
    setEditing(p)
    setForm({ title: p.title || '', category: p.category || 'campus', date: toDateTimeLocal(p.date), imageUrl: p.imageUrl || '' })
    setShowForm(true); setFile(null)
  }
