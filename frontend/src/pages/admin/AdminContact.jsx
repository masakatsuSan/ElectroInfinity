  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <h1 className="font-[Inter,system-ui,sans-serif] font-semibold text-[28px] tracking-tight text-ink">Contact Inbox</h1>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-4 mb-6 border-b border-divider-soft">
        {TABS.map(t => (
          <button key={t} onClick={() => setFilter(t)}
            className={`font-[Inter,system-ui,sans-serif] text-[14px] font-medium px-4 py-2 rounded-full transition-all whitespace-nowrap ${filter === t ? 'bg-primary text-white' : 'bg-soft-stone text-ink-muted-80 hover:text-ink'}`}>
      ...
      {isLoading ? <p className="font-[Inter,system-ui,sans-serif] text-ink-muted-80 text-[15px]">Loading messages�</p>
        ...
        : (
          <div className="border border-divider-soft bg-white rounded-xl overflow-hidden shadow-sm divide-y divide-hairline">
            {contacts.map(c => (
              <div key={c._id} className={`flex items-start gap-4 px-4 sm:px-6 py-3 sm:py-4 hover:bg-[#fff]-parchment transition-colors ${c.status === 'new' ? 'bg-pale-blue/30' : ''}`}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-[15px] font-medium text-ink">{c.name}</p>
                    <span className={`font-mono text-[10px] font-medium uppercase px-2.5 py-1 rounded-full ${c.status === 'new' ? 'bg-pale-blue text-action-blue' : c.status === 'read' ? 'bg-soft-stone text-ink-muted-80' : 'bg-slate-200 text-slate'}`}>{c.status}</span>
                  </div>
                ...
                <div className="flex flex-col gap-1.5 flex-shrink-0">
                  {c.status !== 'archived' && (
                    <button onClick={() => markRead(c)} className="font-[Inter,system-ui,sans-serif] text-[12px] font-medium text-primary bg-primary/10 hover:bg-primary/20 transition-colors px-3 py-1.5 rounded-md">Mark Read</button>
                  )}
                  {c.status !== 'archived' && (
                    <button onClick={() => archive(c)} className="font-[Inter,system-ui,sans-serif] text-[12px] font-medium text-ink-muted-80 hover:text-ink bg-soft-stone hover:bg-soft-stone/70 transition-colors px-3 py-1.5 rounded-md">Archive</button>
                  )}
                  <button onClick={() => toggleReplied(c)} className="font-[Inter,system-ui,sans-serif] text-[12px] font-medium text-ink-muted-80 hover:text-deep-green bg-soft-stone hover:bg-soft-stone/70 transition-colors px-3 py-1.5 rounded-md">{c.isReplied ? 'Unmark Reply' : 'Mark Replied'}</button>
                  <button onClick={() => { if (window.confirm('Delete this message?')) deleteMut.mutate(c._id) }}
                    className="font-[Inter,system-ui,sans-serif] text-[12px] font-medium text-red-500/70 hover:text-red-500 transition-colors bg-red-500/10 hover:bg-red-500/20 px-3 py-1.5 rounded-md">Delete</button>
                </div>
              </div>
            ))}
          </div>
