...
;(async () => {
  ...
  const cases = [
    'Power Systems I — Unit 1 Notes.pdf',
    'my notes (1).pdf',
    'Lecture Notes 2024/25.pdf',
    'Study Material.pdf',
  ]
  ...
  for (const name of cases) {
    ...
    try {
      ...
      console.log(`OK   [${name}] -> ${r.data.data.fileUrl}`)
    } catch (e) {
      console.log(`FAIL [${name}] status=${e.response?.status} data=${JSON.stringify(e.response?.data)}`)
    }
  }
})()