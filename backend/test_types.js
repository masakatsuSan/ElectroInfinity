...
;(async () => {
  ...
  // Current MAIN backend allows: notes, books, organisers, pyqs, yt playlist
  // The FIX branch frontend uses: notes, pyq, assignment, lab_manual, syllabus, other
  const typesToTest = ['pyq', 'assignment', 'lab_manual', 'syllabus', 'other', 'books', 'pyqs']

  for (const t of typesToTest) {
    const fd = new FormData()
    fd.append('file', fs.createReadStream(pdfPath), { contentType: 'application/pdf', filename: 'test.pdf' })
    fd.append('title', 'Test ' + t)
    fd.append('type', t)
    ...
    try {
      ...
      console.log(`${r.status === 201 ? 'OK  ' : 'FAIL'} type='${t}' status=${r.status} ${r.data?.error || ''}`)
    } catch (e) {
      console.log(`ERR  type='${t}' ${e.message}`)
    }
  }
})()