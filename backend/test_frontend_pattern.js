...
;(async () => {
  ...
  // Simulate the EXACT frontend request: FormData with manually-set Content-Type (no boundary)
  ...
  try {
    const r = await axios.post('http://localhost:5000/api/resources', fd, {
      headers: {
        ...
        'Content-Type': 'multipart/form-data', // <-- frontend does this, NO boundary
      },
      maxRedirects: 0,
      validateStatus: () => true,
    })
